...
:::info
本功能的仅限于光启亚比计算伤害使用,其它时代亚比由于伤害计算公式与光启时代亚比不完全相同,因此不能完全适用于该伤害计算器.

该伤害计算器采用的数字格式是小数,而不是百分数,使用时请注意数字格式的转变.
:::

::: warning
其中**伤害加成|爆伤加成|数值加成**3部分较为复杂且具有争议,误差和实际相对较大.
:::
...
![placeholder](/aola/damage_004.png)（局外）
![placeholder](/aola/damage_005.png)（局内）取固定值2.8/3.8.
...
![placeholder](/aola/damage_006.png)（具体克制倍率加成数值看局外群星,满配为150%,固定取值150%即可）
...
![placeholder](/aola/damage_008.png)(局外)
![placeholder](/aola/damage_009.png)（局内）
...
- `att` 为我方攻击(特攻)属性等级, `def` 为敌方防御(特防)属性等级, $\{ att,def \in \mathbb{Z} \mid -10 \leq att,def \leq 10 \}$.
- 当 `att > 0` 时, $att1 = att, att2 = 0$.
- 当 `att < 0` 时, $att1 = 0, att2 = att$.
- 当 `def > 0` 时, $def1 = 0, def2 = def$.
- 当 `def < 0` 时, $def1 = def, def2 = 0$.

公式逻辑遵循以下原则：

- 我方攻击(特攻)属性 `att`：正值 (`>0`) 作用于分子,负值 (`<0`) 的绝对值作用于分母.
- 敌方防御(特防)属性 `def`：正值 (`>0`) 作用于分母,负值 (`<0`) 的绝对值作用于分子.
- 如果属性等级为0,则该项不产生影响.

::: tip
属性等级为**我方+6敌方-6**时,数值为**6.25**.
:::

下面是该公式的 Python 实现及一个计算示例：

```python
def calculate_attribute_bonus(x1, x2):
    """
    参数:
    x1 (int): 我方攻击属性等级 (-10 到 10)
    x2 (int): 敌方防御属性等级 (-10 到 10)

    返回:
    float: 伤害加成系数
    """
    if not -10 <= x1 <= 10 or not -10 <= x2 <= 10:
        raise ValueError("属性等级必须在 -10 到 10 之间")
...
# --- 示例 ---
# 假设我方特攻属性等级为 +6, 敌方特防属性等级为 -3
our_attack_level = 6
enemy_defense_level = -3

bonus = calculate_attribute_bonus(our_attack_level, enemy_defense_level)

print(f"我方特攻属性等级: {our_attack_level}")
print(f"敌方特防属性等级: {enemy_defense_level}")
print(f"计算出的伤害加成系数为: {bonus}")
...
![placeholder](/aola/damage_026.png)![placeholder](/aola/damage_027.png)（局外爆伤面板）
